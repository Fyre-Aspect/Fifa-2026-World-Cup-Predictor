...
const sumWeights = (w: ModelWeights) => w.elo + w.form + w.squad + w.polymarket + w.books;
...
describe('gradientStep', () => {
  it('keeps weights on the simplex (sum to 1, non-negative)', () => {
    const { weights } = gradientStep(WEIGHTS, inputs({ elo: homeHeavy, form: awayHeavy }), 'away');
    expect(sumWeights(weights)).toBeCloseTo(1, 10);
    for (const k of ['elo', 'form', 'squad', 'polymarket', 'books'] as const) {
  ...
  it('shifts weight toward the input that was more correct', () => {
    ...
    const { weights } = gradientStep(WEIGHTS, inputs({ elo: homeHeavy, form: awayHeavy }), 'away', 0.1);
  ...
  it('does not move absent inputs much beyond renormalization', () => {
    ...
    const { weights } = gradientStep(WEIGHTS, inputs({ elo: homeHeavy }), 'home', 0.1);
  ...
  it('reports the blended Brier and per-input Brier', () => {
    const step = gradientStep(WEIGHTS, inputs({ elo: homeHeavy, form: awayHeavy }), 'home');
